...
window.addEventListener("load", () => {
  Swal.fire({
    title: "คุณต้องการเปิดเพลงประกอบไหม?",
    icon: "warning",
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const targetDay = 4;
  const targetMonthIndex = 6; // July (0-indexed)
  ...
  const interval = setInterval(() => {
    // Update day, month, year in the DOM
    dayElement.textContent = currentDay;
    monthElement.textContent = months[currentMonthIndex];
    yearElement.textContent = currentYear;

    // Smoothly increment day, month, year
...
const animationTimeline = () => {
  ...
  tl.to(".container", 0.6, {
    ...
    .call(() => {
      // เริ่ม Vanta Birds effect เมื่อถึงหน้าสุดท้าย
